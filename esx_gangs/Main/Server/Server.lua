local DISCORD_WEBHOOK = "https://discord.com/api/webhooks/810580809206857749/SludIfaIkpbrxXh6sgJbeUw7KnPfddf3L527RGkkqr0xv7HE9lw4iLEivxgfxuAsiL6H" -- Place Webhook
local DISCORD_NAME = "Turf Wars (Grind)" -- Place Bot Name
local DISCORD_IMAGE = "https://cdn.discordapp.com/attachments/828801766350389259/829141746599985192/glrp250.png" -- Place Server Logo
...
local function chatMessage(target, author, msg)
    TriggerClientEvent('chat:addMessage', target, { --148, 0, 211
        template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(100, 0, 200, 0.9); border-radius: 3px;"><i class="fas fa-skull-crossbones"></i> '..author..':<br> {1}<br></div>',
        args = { author, msg }
    })
end
...
local function InsertPlayer(ident, gang_name, ranking) 
    ...
    chatMessage(-1, "[^1Gangs^0]", player.firstname .. " " .. player.lastname .. " was set as ^2".. gang_data.label .. ": ".. rank_data.label .."^0.")
...
local function InvitePlayer(ident, gang_name)
    ...
    local player = tplayer[1]
    local gang_data = gangs[gang_name]
    chatMessage(-1, "[^1Gangs^0]", player.firstname .. " " .. player.lastname .. " was ^2accepted^0 into ^2".. gang_data.label .. "^0.")
...
local function FirePlayer(ident)
    ...
    chatMessage(-1, "[^1Gangs^0]", player.firstname .. " " .. player.lastname .. " was ^1removed^0 from ^1".. gang_data.label .. "^0.")
...
local function PromotePlayer(ident)
    ...
    if (getRankData(player.gang, player.gang_rank + 1) ~= nil) then
        chatMessage(-1, "[^1Gangs^0]", "Promoted " .. player.firstname .. " " .. player.lastname .. " to ^2".. getRankData(player.gang, player.gang_rank + 1).label .. "^0.")
        ...
        chatMessage(-1, "[^1Gangs^0]", "Couldn't promote " .. player.firstname .. " " .. player.lastname .. " any higher.")
    end
end
...
local function DemotePlayer(ident)
    ...
    local gang_data = gangs[player.gang]
    if (getRankData(player.gang, player.gang_rank - 1) ~= nil) then
        chatMessage(-1, "[^1Gangs^0]", "Demoted " .. player.firstname .. " " .. player.lastname .. " to ^1".. getRankData(player.gang, player.gang_rank - 1).label .. "^0.")
        MySQL.Sync.execute("UPDATE users SET gang_rank=@gang_rank WHERE identifier=@identifier", {['@gang_rank'] = player.gang_rank - 1, ['@identifier'] = ident})
        if (ESX.GetPlayerFromIdentifier(ident) ~= nil) then
            local xPlayer = ESX.GetPlayerFromIdentifier(ident)
            if (xPlayer.source ~= nil) then
        ...
        chatMessage(-1, "[^1Gangs^0]", "Couldn't demote " .. player.firstname .. " " .. player.lastname .. " any lower.")
    end
end
        ...
        if (_item == "cash" or _item == "dcash") then 
            ...
            if (count - amount >= 0) then
                if (_item == "cash") then 
                    ...
                    TriggerEvent("Discord:ToDiscord", "Money Deposited", "**Name: **"..GetPlayerName(xLeader.source).." (ID: "..tonumber(leader)..") ("..steamid..")\n**Amount: **$"..amount.."\n**Gang: **"..gang_data.label.."\n**Date & Time: **"..(os.date("%B %d, %Y at %I:%M %p")), "https://discord.com/api/webhooks/824834033569628171/eMKFpfHzEDdC63UCK3nWcQUe9o-pO41vHkzu2nI_cAUot_RoMwcTObr_EuW9XzMcg7hy", 'Gang Storage')
                else 
                    ...
                    TriggerEvent("Discord:ToDiscord", "Dirty Money Deposited", "**Name: **"..GetPlayerName(xLeader.source).." (ID: "..tonumber(leader)..") ("..steamid..")\n**Amount: **$"..amount.."\n**Gang: **"..gang_data.label.."\n**Date & Time: **"..(os.date("%B %d, %Y at %I:%M %p")), "https://discord.com/api/webhooks/824834033569628171/eMKFpfHzEDdC63UCK3nWcQUe9o-pO41vHkzu2nI_cAUot_RoMwcTObr_EuW9XzMcg7hy", 'Gang Storage')
                end
            ...
            if (item.count - amount >= 0) then
                ...
                TriggerEvent("Discord:ToDiscord", "Item Deposited", "**Name: **"..GetPlayerName(xLeader.source).." (ID: "..tonumber(leader)..") ("..steamid..")\n**Item: **"..item.label.."\n**Amount: **x"..amount.."\n**Gang: **"..gang_data.label.."\n**Date & Time: **"..(os.date("%B %d, %Y at %I:%M %p")), "https://discord.com/api/webhooks/824834033569628171/eMKFpfHzEDdC63UCK3nWcQUe9o-pO41vHkzu2nI_cAUot_RoMwcTObr_EuW9XzMcg7hy", 'Gang Storage')
            end
        end
    end
end
        ...
        if (_item == "cash" or _item == "dcash") then 
            ...
            if (count ~= nil and count - amount >= 0) then 
                if (_item == "cash") then 
                    ...
                    TriggerEvent("Discord:ToDiscord", "Money Withdrawn", "**Name: **"..GetPlayerName(xLeader.source).." (ID: "..tonumber(leader)..") ("..steamid..")\n**Amount: **$"..amount.."\n**Gang: **"..gang_data.label.."\n**Date & Time: **"..(os.date("%B %d, %Y at %I:%M %p")), "https://discord.com/api/webhooks/824834033569628171/eMKFpfHzEDdC63UCK3nWcQUe9o-pO41vHkzu2nI_cAUot_RoMwcTObr_EuW9XzMcg7hy", 'Gang Storage')
                else 
                    xLeader.addAccountMoney('black_money', amount)
                    gangs[gang_name].inventory.dcash = gangs[gang_name].inventory.dcash - amount
                    TriggerEvent("Discord:ToDiscord", "Dirty Money Withdrawn", "**Name: **"..GetPlayerName(xLeader.source).." (ID: "..tonumber(leader)..") ("..steamid..")\n**Amount: **$"..amount.."\n**Gang: **"..gang_data.label.."\n**Date & Time: **"..(os.date("%B %d, %Y at %I:%M %p")), "https://discord.com/api/webhooks/824834033569628171/eMKFpfHzEDdC63UCK3nWcQUe9o-pO41vHkzu2nI_cAUot_RoMwcTObr_EuW9XzMcg7hy", 'Gang Storage')
                end
                UpdateItems(gang_name)
            else
                TriggerClientEvent('esx:showNotification', leader, "~r~You dont have enough.~w~")
            ...
            if (item ~= nil and item - amount >= 0) then
                ...
                UpdateItems(gang_name)
                TriggerEvent("Discord:ToDiscord", "Item Withdrawn", "**Name: **"..GetPlayerName(xLeader.source).." (ID: "..tonumber(leader)..") ("..steamid..")\n**Item: **"..thing.label.."\n**Amount: **x"..amount.."\n**Gang: **"..gang_data.label.."\n**Date & Time: **"..(os.date("%B %d, %Y at %I:%M %p")), "https://discord.com/api/webhooks/824834033569628171/eMKFpfHzEDdC63UCK3nWcQUe9o-pO41vHkzu2nI_cAUot_RoMwcTObr_EuW9XzMcg7hy", 'Gang Storage')
            else
                TriggerClientEvent('esx:showNotification', leader, "~r~You dont have enough.~w~")
            end
        end
...
local function FinishCapturing(_zone)
    ...
    if (highest.name == "") then
        ...
        chatMessage(-1, "[^1Turf Wars^0]", 'The turf "^1'.. zone.Label ..'^0" was captured by ^1'.. winner_label..'^0! (Rewards: '.. amount ..'.')
        sendToDiscord("GrindLife TURF LOGS", "" .. winner_label .. " Captured the Turf ".. zone.Label ..".", color)
    end
    ...
    if (isGangLeader(leader, l_gang)) then
        ...
        if (#t_player == 1) then
            ...
            if (l_rank.ranking > player.gang_rank + 1) then
                ...
                chatMessage(leader, "[^1Gangs^0]", "Couldn't promote " .. player.firstname .. " " .. player.lastname .. " any higher.")
            end
        end
    end
end)
...
AddEventHandler("esx_gangs:InvitePlayer", function(ident) 
    ...
    if (i_gang == nil) then
        ...
        chatMessage(leader, "[^1Gangs^0]", "That person is already in another gang.")
    end
end)
    ...
    if (isGangLeader(leader, l_gang)) then
        ...
        if (#t_player == 1) then
            ...
            if (l_rank.ranking > player.gang_rank and 0 <= player.gang_rank - 1) then
                ...
                chatMessage(leader, "[^1Gangs^0]", "Couldn't demote " .. player.firstname .. " " .. player.lastname .. " any lower.")
            end
        end
    end
end)
    ...
    if (zones_waiting[_zone] == nil or zones_waiting[_zone] <= 0) then
        if (zone ~= nil and (gang ~= nil)) then
            if (not zone.capturing) then 
                ...
                chatMessage(-1, "[^1Turf Wars^0]", '^0The turf "^1'.. zone.Label ..'^0" is now being ^0captured! It will be over in ^1'.. Config.CaptureTimer ..' second(s)^0!')
                zones[_zone] = zone
                TriggerClientEvent("esx_gangs:UpdateZones", -1, zones)
            end
        else
...
RegisterCommand("setgang", function(src, args, raw)
    ...
    if (isAdmin(source)) then 
        ...
        if (target ~= nil and gang ~= nil and rank ~= nil) then 
            ...
            chatMessage(source, "[^1Gangs^0]", "^1USAGE: ^3/setgang [player_id] [gang_name] [#rank]")
        end
    else
        chatMessage(source, "[^1Gangs^0]", "^1You don't have permission to use this command.")
    end
end, false)
